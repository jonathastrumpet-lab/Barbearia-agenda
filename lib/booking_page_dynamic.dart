...
class _BookingPageState extends State<BookingPage> {
  ...
  Future<void> _loadBarbers() async {
    ...
    try {
      final barbers = await BarberStore.loadActive();
      if (!mounted) return;
      setState(() {
        _barbers = barbers;
        _barberIndex = 0;
      ...
      if (barbers.isNotEmpty) await _refreshAvailability();
    } catch (_) {
      if (!mounted) return;
      setState(() {
        _barbers = const [];
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Não foi possível carregar os barbeiros desta barbearia.'),
  ...
  Future<void> _confirmBooking() async {
    if (_barbers.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Esta barbearia ainda não possui barbeiros ativos.'),
          behavior: SnackBarBehavior.floating,
        ),
      );
      return;
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Esse horário acabou de ser reservado. Escolha outro horário.'),
          behavior: SnackBarBehavior.floating,
        ),
      );
      return;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Não foi possível sincronizar o agendamento com a nuvem.'),
          behavior: SnackBarBehavior.floating,
        ),
      );
      return;
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(child: _sectionTitle('2', 'Barbeiro')),
                    if (_loadingBarbers)
                      const SizedBox(
                        width: 18,
                        height: 18,
                ...
                const SizedBox(height: 12),
                if (!_loadingBarbers && _barbers.isEmpty)
                  Container(
                    ...
                    child: const Text(
                      'Nenhum barbeiro ativo nesta barbearia no momento.',
                  ...
                  SizedBox(
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ChoiceChip(
                          selected: selected,
                          label: Text(_barbers[index].name),
                          avatar: Icon(
                ...
                Material(
                  ...
                  child: InkWell(
                    borderRadius: BorderRadius.circular(16),
                    onTap: _barbers.isEmpty ? null : _pickDate,
                    child: Padding(
                      padding: const EdgeInsets.all(16),
                      child: Row(
                        children: [
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        selectedBarber == null
                            ? 'Selecione uma barbearia com barbeiro ativo'
                ...
                SizedBox(
                  ...
                  child: FilledButton.icon(
                    onPressed: _saving || _barbers.isEmpty ? null : _confirmBooking,
                    style: FilledButton.styleFrom(
                      backgroundColor: _gold,
                      foregroundColor: Colors.black,
                    ...
                    label: Text(
                      _saving ? 'SINCRONIZANDO...' : 'CONFIRMAR AGENDAMENTO',
