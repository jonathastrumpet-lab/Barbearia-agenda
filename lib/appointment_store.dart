...
class Appointment {
  ...
  final String status;
  final String? cancelledBy, cancelledAtIso;

  DateTime get date => DateTime.parse(dateIso);
  bool get isCancelled => status == 'cancelled';

  Appointment copyWith({String? status, String? cancelledBy, String? cancelledAtIso}) => Appointment(
        id: id,
        service: service,
        duration: duration,
        price: price,
        ...
        cancelledAtIso: cancelledAtIso ?? this.cancelledAtIso,
      );
  ...
  factory Appointment.fromJson(Map<String, dynamic> j) => Appointment(
        ...
        cancelledAtIso: j['cancelledAtIso']?.toString(),
      );
  ...
  factory Appointment.fromCloudJson(Map<String, dynamic> j) => Appointment(
        id: j['id'].toString(),
        service: j['service']?.toString() ?? '',
        duration: j['duration']?.toString() ?? '',
        price: j['price']?.toString() ?? '',
        barber: j['barber']?.toString() ?? '',
        dateIso: j['date_iso']?.toString() ?? '',
        time: j['time']?.toString() ?? '',
        createdAtIso: j['created_at_iso']?.toString() ?? '',
        status: j['status']?.toString() ?? 'scheduled',
        cancelledBy: j['cancelled_by']?.toString(),
        cancelledAtIso: j['cancelled_at']?.toString(),
      );
}
...
class AppointmentStore {
  ...
  static Future<void> initializeCloud() async {
    ...
    try {
      ...
      if (client.auth.currentUser != null) {
        ...
        _cloudError = 'Entre com sua conta.';
      }
    } catch (e, s) {
      _cloudReady = false;
      _cloudError = e.toString();
      debugPrint('[Supabase] Falha ao inicializar: $e');
  ...
  static Future<bool> signUp(String email, String password) async {
    _cloudReady = false;
    _cloudError = null;
    final r = await client.auth.signUp(email: email.trim(), password: password);
    if (r.session != null) {
      await refreshMembership();
      return true;
    }
  ...
  static Future<void> refreshMembership() async {
    ...
    if (metadata['account_type'] == 'barbershop_admin') {
      _cloudError = 'Conta da barbearia ainda nao configurada.';
      return;
    }
  ...
  static Future<void> claimOwnerAccess(String inviteCode) async {
    if (!signedIn) throw StateError('Faca login primeiro.');
    await client.rpc('claim_barbershop_owner', params: {'invite_code': inviteCode.trim()});
    await refreshMembership();
  }
  ...
  static Future<List<Appointment>> load() async {
    if (cloudEnabled) {
      try {
        final r = await _loadRemote();
        await _save(r);
        return r;
      } catch (e, s) {
        _cloudError = e.toString();
        debugPrint('[Supabase] Falha ao carregar agenda: $e');
        debugPrintStack(stackTrace: s);
      }
    }
    return _loadLocal();
  ...
  static Future<void> add(Appointment a) async {
    if (!cloudEnabled) throw StateError(_cloudError ?? 'Agenda na nuvem indisponivel.');
    try {
      await _addRemote(a);
      _cloudError = null;
    } catch (e, s) {
      _cloudError = e.toString();
      debugPrint('[Supabase] Falha ao inserir agendamento: $e');
      debugPrintStack(stackTrace: s);
      rethrow;
    }
  ...
  static Future<void> remove(String id) async {
    if (!cloudEnabled) throw StateError(_cloudError ?? 'Agenda na nuvem indisponivel.');
    final cancelledBy = isAdmin ? 'admin' : 'client';
    final cancelledAtIso = DateTime.now().toUtc().toIso8601String();
    try {
      await _removeRemote(id, cancelledBy, cancelledAtIso);
      _cloudError = null;
    } catch (e, s) {
      _cloudError = e.toString();
      debugPrint('[Supabase] Falha ao cancelar agendamento: $e');
      debugPrintStack(stackTrace: s);
      rethrow;
    }
    final items = await _loadLocal();
  ...
  static Future<List<Appointment>> _loadLocal() async {
    ...
    try {
      ...
      final items = d.map((x) => Appointment.fromJson(x as Map<String, dynamic>)).toList();
      items.sort(_compareAppointments);
      return items;
    } catch (_) {
      return [];
  ...
  static Future<List<Appointment>> _loadRemote() async {
    final user = client.auth.currentUser, shop = shopId;
    if (user == null) throw StateError('Sessao Supabase nao autenticada.');
    if (shop == null) throw StateError('Estabelecimento nao selecionado.');
    var q = client
        .from('appointments')
        .select('id,service,duration,price,barber,date_iso,time,created_at_iso,status,cancelled_by,cancelled_at')
        .eq('barbershop_id', shop);
    if (!isAdmin) q = q.eq('client_id', user.id);
    final rows = await q.order('date_iso').order('time');
    final items = (rows as List<dynamic>).map((x) => Appointment.fromCloudJson(x as Map<String, dynamic>)).toList();
    items.sort(_compareAppointments);
    return items;
  }

  static Future<void> _addRemote(Appointment a) async {
    final user = client.auth.currentUser;
    if (user == null) throw StateError('Sessao Supabase nao autenticada.');
    try {
      await client.from('appointments').insert(a.toCloudJson(user.id));
    } on PostgrestException catch (e) {
      if (e.code == '23505') throw const AppointmentConflictException();
      rethrow;
    }
  }

  static Future<void> _removeRemote(String id, String cancelledBy, String cancelledAtIso) async {
    final shop = shopId, user = client.auth.currentUser;
    if (user == null) throw StateError('Sessao Supabase nao autenticada.');
    if (shop == null) throw StateError('Estabelecimento nao selecionado.');
    var q = client.from('appointments').update({
      'status': 'cancelled',
  ...
  static Future<void> _save(List<Appointment> items) async {
    final p = await SharedPreferences.getInstance();
    await p.setString(_key, jsonEncode(items.map((x) => x.toJson()).toList()));
  }

  static int _compareAppointments(Appointment a, Appointment b) {
    DateTime v(Appointment x) {
      final p = x.time.split(':');
      return DateTime(x.date.year, x.date.month, x.date.day, int.parse(p[0]), int.parse(p[1]));
    }
