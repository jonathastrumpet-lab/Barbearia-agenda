...
class _ScheduleSettingsPageState extends State<ScheduleSettingsPage> {
  ...
  Future<void> _load() async {
    ...
    try{
      final results=await Future.wait<dynamic>([
        ...
        BarberStore.loadAll(),
        AppointmentStore.client.from('barber_custom_hours').select('barber_id,weekday,enabled,start_minutes,end_minutes,break_enabled,break_start_minutes,break_end_minutes').eq('barbershop_id',shop).order('weekday'),
      ]);
      ...
      if(rows.isNotEmpty){
        ...
        _days=[for(final f in _days) if(by[f.weekday]==null) f else _fromRow(f.weekday,by[f.weekday]!)];
      }
      _barbers=results[1] as List<BarberRecord>;
      final customRows=results[2] as List<dynamic>;
      final grouped=<String,Map<int,Map<String,dynamic>>>{};
      for(final raw in customRows){final r=raw as Map<String,dynamic>;final id=r['barber_id'].toString();(grouped[id]??=<int,Map<String,dynamic>>{})[r['weekday'] as int]=r;}
      for(final b in _barbers){
        _barberCustom[b.id]=b.usesCustomSchedule;
        final saved=grouped[b.id];
        _barberDays[b.id]=[for(final base in _days) saved?[base.weekday]==null?base.copyWith():_fromRow(base.weekday,saved![base.weekday]!)];
      }
  ...
  DaySchedule _fromRow(int weekday,Map<String,dynamic> r)=>DaySchedule(weekday:weekday,enabled:r['enabled']==true,startMinutes:r['start_minutes'] as int?,endMinutes:r['end_minutes'] as int?,breakEnabled:r['break_enabled']==true,breakStartMinutes:r['break_start_minutes'] as int?,breakEndMinutes:r['break_end_minutes'] as int?);
  ...
  Map<String,dynamic> _row(String shop,String? barber,DaySchedule d,String now){final r=<String,dynamic>{'barbershop_id':shop,'weekday':d.weekday,'enabled':d.enabled,'start_minutes':d.startMinutes,'end_minutes':d.endMinutes,'break_enabled':d.breakEnabled,'break_start_minutes':d.breakStartMinutes,'break_end_minutes':d.breakEndMinutes,'updated_at':now};if(barber!=null)r['barber_id']=barber;return r;}
  ...
  Future<void> _saveHours()async{
    ...
    try{
      ...
      await AppointmentStore.client.from('barbershop_business_hours').upsert(_days.map((d)=>_row(shop,null,d,now)).toList(),onConflict:'barbershop_id,weekday');
      for(final b in _barbers){
        final custom=_barberCustom[b.id]??false;
        await BarberStore.setUsesCustomSchedule(b.id,custom);
        if(custom){await AppointmentStore.client.from('barber_custom_hours').upsert(_barberDays[b.id]!.map((d)=>_row(shop,b.id,d,now)).toList(),onConflict:'barbershop_id,barber_id,weekday');}
        else{await AppointmentStore.client.from('barber_custom_hours').delete().eq('barbershop_id',shop).eq('barber_id',b.id);}
      }
      if(!mounted)return;ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Horários salvos com sucesso.')));
